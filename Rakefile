...
task :wishlist do
  ...
  open('wishlist.md', 'w'){|f|
    ...
    wishlist.each{|item|
      ...
      q = CGI.escape(name)
      f.print "[#{label}](http://www.bookfinder.com/search/?title=#{q}&submitBtn=Search&new_used=U&destination=nl&currency=EUR&binding=*&isbn=&keywords=&minprice=&maxprice=&min_year=&max_year=&mode=advanced&st=sr&ac=qr)"
